...
import { ModuleHelp, StatusHelp } from '../components/ui/Help';
...
export function OverviewPage({ summary }: { summary: AssuranceSummary; current?: Assessment }) {
  ...
  return <div className="overview-workspace overview-page">
    <section className="overview-assurance" aria-label="Overall assurance"><span>OVERALL ASSURANCE</span><div role="img" aria-label={summary.overall.assurance_score === null ? 'Assurance score unavailable' : `Backend assurance score ${summary.overall.assurance_score.toFixed(1)} out of 100`}><strong>{summary.overall.assurance_score === null ? 'UNAVAILABLE' : `${summary.overall.assurance_score.toFixed(1)} / 100`}</strong><StatusBadge value={summary.overall.disposition} /></div></section>
    <section className="overview-modules" aria-label="Assurance modules">{moduleOrder.map((key) => { const item = summary.modules[key]; const Icon = moduleIcons[key]; return <article key={key} className={`overview-module module-${item.status.toLowerCase()}`}><span>{key === 'distribution_shift' ? 'Distribution Shift' : item.display_name}</span><ModuleHelp module={key} /><strong><Icon />{item.status.replaceAll('_', ' ')}</strong><small>{item.availability === 'UNKNOWN' ? item.availability_reason : `${item.finding_count} ${item.finding_count === 1 ? 'finding' : 'findings'} · ${item.availability}`}</small></article>; })}</section>
