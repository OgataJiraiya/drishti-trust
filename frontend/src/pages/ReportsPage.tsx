...
export function ReportsPage({ summary, findings }: { summary: AssuranceSummary; findings: Finding[] }) {
  return <div className="reference-page report-page">
    ...
    <section className="reference-section attention-list"><span>FINDINGS REQUIRING ATTENTION ({Math.min(findings.length, 5)} FINDINGS)</span>{findings.slice(0, 5).map((finding) => <div className={`report-finding severity-${finding.severity.toLowerCase()}`} key={finding.finding_id}><code>{finding.finding_id}</code><strong>{finding.category.replaceAll('_', ' ')}</strong><StatusBadge value={finding.severity} /><StatusBadge value={finding.recommendation} /></div>)}</section>
