...
describe('distribution shift', () => {
  it('renders honest live unavailability and no global drift score', () => { render(<DistributionPage summary={summary} findings={findings} demo={false} />); expect(screen.getByText(/Detailed detector evidence unavailable in live mode/i)).toBeVisible(); expect(screen.queryByText(/drift score/i)).not.toBeInTheDocument(); });
