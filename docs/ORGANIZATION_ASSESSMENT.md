...
Detectors run before creating the backend assessment. Successful detector orchestration creates DRAFT, activates it, signs each submitted module with a fresh in-memory Ed25519 key, authenticates through `ModuleAuthService`, and submits through the unchanged integration gate. Backend `AssessmentService` owns sealing; `SummaryService` owns scores and coverage. Private execution keys never leave the server. Public producer registrations remain auditable. The model registry upload/verify APIs remain identity-only and are not used for execution.

An existing ACTIVE assessment is not modified automatically; activation conflicts fail honestly. A backend failure after lifecycle creation may leave DRAFT/ACTIVE evidence for administrative investigation. The UI reports failure and the last known lifecycle, never substitutes fixtures or seals incomplete work as successful. Audit delivery failure can occur after sealing; consult System/audit status. The snapshot records audit state at sealing rather than claiming a future audit result.
